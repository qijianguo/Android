...
public class WuhanYunweiBaogaoActivity extends Activity {
	...
	private Button yunwei_baogao_goback;
			
	private EditText yunwei_baogao_et_query_time;
	private String query_time;
	
	private ListView yunwei_baogao_list_view;
	...
	private ArrayList<String> alert_type_list = new ArrayList<String>();
	...
	public boolean onKeyDown(int keyCode, android.view.KeyEvent event) {
		if (keyCode == KeyEvent.KEYCODE_BACK && event.getRepeatCount() == 0) {
			Intent intent = new Intent(WuhanYunweiBaogaoActivity.this, WuhanMainActivity.class);
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_wuhan_yunwei_baogao);

		yunwei_baogao_list_view = (ListView) findViewById(R.id.yunwei_baogao_list_view);

		yunwei_baogao_goback = (Button) findViewById(R.id.yunwei_baogao_goback);
		yunwei_baogao_goback.setOnClickListener(new OnClickListener() {
			@Override
		...
		yunwei_baogao_et_query_time = (EditText) findViewById(R.id.yunwei_baogao_et_query_time);
		Calendar calendar = Calendar.getInstance();
		query_time = new SimpleDateFormat("yyyy-MM-dd").format(calendar.getTime());
		yunwei_baogao_et_query_time.setText(query_time);
		yunwei_baogao_et_query_time.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
				DatePickDialogUtil dataTimePickDialog = new DatePickDialogUtil(
						WuhanYunweiBaogaoActivity.this, query_time);
				dataTimePickDialog
						.dateTimePicKDialog(yunwei_baogao_et_query_time);
			}
		});
		yunwei_baogao_et_query_time.addTextChangedListener(new TextWatcher() {
	...
	protected void getListInfo() {
		...
		new Thread() {
			...
			public void run() {
				...
				String json = null;
				int responseCode = 0;
				String query_str = yunwei_baogao_et_query_time.getText().toString();
				query_time = query_str;
				if (query_str.length() >= 16) {
					query_str = query_str.substring(0,10)+"%20"+query_str.substring(11,16)+":28";
				}
				String query_path = LoginState.getIns().getServerURL() + "getMaintenReportList?start_time="+query_str;
				...
				try {
					...
					if (responseCode == 200) {
						...
						for (int i = 0; i < jsonArray.length(); i++) {
							...
							alert_type_list.add(item.getString("alert_type"));
							color_type_list.add(item.getString("alert_mx_type"));
						}
	...
	private ArrayList<Map<String,Object>> getListItems(){
		...
		for(int i = 0;i < rank_list.size();i++){
			...
			map.put("content", alert_type_list.get(i));
	...
	private void showList(){
		...
		normalListViewAdapter = new NormalListViewAdapter(WuhanYunweiBaogaoActivity.this,listItems,4,6,5,16,16,16);
		yunwei_baogao_list_view.setAdapter(normalListViewAdapter);
		yunwei_baogao_list_view.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				Intent intent = new Intent(WuhanYunweiBaogaoActivity.this,NewSpotDetailsActivity.class);
	...
	private void showProgress() {
		UIUtil.showProgressDialog(WuhanYunweiBaogaoActivity.this, "搜索中......");
	}
	...
	protected void showNetworkError() {
		Toast toast = Toast.makeText(WuhanYunweiBaogaoActivity.this, "亲，你还没连接网络呢= =!",
				Toast.LENGTH_SHORT);
		toast.show();
	}

	protected void showServerError() {
		Toast toast = Toast.makeText(WuhanYunweiBaogaoActivity.this, "服务器维护中，请稍后",
				Toast.LENGTH_SHORT);
		toast.show();
	}
}
