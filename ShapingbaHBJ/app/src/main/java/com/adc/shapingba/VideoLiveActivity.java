...
public class VideoLiveActivity extends Activity implements OnClickListener,
	...
	private int idx = NewSpotDetailsActivity.idx;
	...
	*/
	...
	private void initData() {
		...
		//现在video_service_ip加了端口号，所以要去掉最后":xxxx"5个字符取子串
		String video_service_ip_str = spotInfos.get(idx).getVideo_service_ip();
		int len = video_service_ip_str.length();
		/*if(len >= 5){
			temp_info.acsIP = video_service_ip_str.substring(0,len-5);
		}*/
		temp_info.acsIP = video_service_ip_str;
	...
	private void initUI() {
		...
		//手机端默认码流类型MAG getSpotListByUser中返回的video_phone_mag
		//MAG=0,子码流=1,主码流=2
		
		mStreamType = ConstantLive.MAG;
		String videoStreamType = spotInfos.get(idx).getVideo_phone_mag(); 
		if(videoStreamType != null){
			if(videoStreamType.equals("2")){
				mStreamType = ConstantLive.MAG;
			}else if (videoStreamType.equals("1")) {
				mStreamType = ConstantLive.SUB_STREAM;
			}else if (videoStreamType.equals("0")) {
