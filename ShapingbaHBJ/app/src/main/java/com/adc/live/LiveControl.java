...
import com.adc.util.UtilSDCard;
...
public class LiveControl implements RtspClientCallback, PlayerDisplayCB {
	...
	public boolean capture(String filePath, String picName) {
		if (!UtilSDCard.isSDCardUsable()) {
			if (null != mLiveCallBack) {
				mLiveCallBack
						.onMessageCallback(ConstantLive.SD_CARD_UN_USEABLE);
			}
			return false;
		}
		...
		if (UtilSDCard.getSDCardRemainSize() <= mSDCardSize) {
			if (null != mLiveCallBack) {
				mLiveCallBack
						.onMessageCallback(ConstantLive.SD_CARD_SIZE_NOT_ENOUGH);
			}
			return false;
		}
	...
	private boolean writeStreamHead(File file) {
		if (null == file || null == mStreamHeadDataBuffer) {
			if (file == null)
				Log.i("heheda", "fail--111111111111111111");
			if (mStreamHeadDataBuffer == null)
				Log.i("heheda", "fail-22222222222222");
			return false;
		}

		byte[] tempByte = mStreamHeadDataBuffer.array();
		if (null == tempByte) {
			Log.i("heheda", "fail2222222222222222222222");
		...
		try {
			...
			Log.i("heheda", "fail33333333333333333333333");
			e.printStackTrace();
			mRecordFileOutputStream = null;
			mStreamHeadDataBuffer = null;
			return false;
